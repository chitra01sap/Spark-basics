...
object MostPopularSuperheroDataset {
  ...
  def main(args: Array[String]) {
    ...
    val singleConnection: DataFrame =
      connections.filter($"connections" === 1).toDF()

    singleConnection.show()
    ...
    println(
      s"${mostPopularName(0)} is the most popular superhero with ${mostPopular(1)} co-appearances.")
  }
}
