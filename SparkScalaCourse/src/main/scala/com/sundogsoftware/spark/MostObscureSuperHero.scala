...
object MostObscureSuperHero {
  ...
  def main(args: Array[String]) {
    ...
    val minConnection: Dataset[Row] =
      connections.filter($"connections" === 1)

    minConnection.show()

    val connectionNale = minConnection.join(names, usingColumn = "id")

    connectionNale.select("name").show()
  }
}
